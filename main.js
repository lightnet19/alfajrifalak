/**
 * prayer.js — Al-Fajri
 * Kalkulasi & Render Waktu Sholat — Metode Kemenag (Fajr 20°, Isha 18°)
 */
'use strict';

let _pCache = { result: null, dateKey: null };

function prayerTimes(year, month, day, lat, lng, tz, elev) {
  // Selalu pakai noon UT agar posisi matahari konsisten
  const j0 = jd(year, month, day, 12 - tz, 0, 0);
  const { Dec, EqT } = sunPos(j0);
  const noon     = 12 - lng / 15 - EqT / 60 + tz;
  const elevCorr = 0.8333 + 0.0347 * Math.sqrt(elev || 0);

  function ha(altDeg) {
    const c = (sin(altDeg) - sin(lat) * sin(Dec)) / (cos(lat) * cos(Dec));
    return Math.abs(c) > 1 ? null : acos(c) / 15;
  }
  // Ashar Syafi'i: shadow = 1× tinggi benda
  const ashrAlt = (Math.PI / 2 - Math.atan(Math.tan(Math.abs(lat - Dec) * D2R) + 1)) * R2D;

  const fajr = ha(-20), syuruq = ha(-elevCorr), maghrib = ha(-elevCorr), isya = ha(-18), ashr = ha(ashrAlt);
  return {
...
function renderPrayer() {
  ...
  const mins   = prayers.map(x => { if(x.t==='—')return Infinity; const[h,m]=x.t.split(':').map(Number); return h*60+m; });
  let curIdx = -1;
  for (let i = 0; i < mins.length; i++) if (mins[i] <= nowMin) curIdx = i;
...
// Countdown — dipanggil tiap detik, hanya 5 waktu wajib
