...
export function decimalHoursToTimeString(value) {
  if (value == null || Number.isNaN(value)) return "-";
  const h = Math.floor(normalize24(value));
  const m = Math.floor((normalize24(value) - h) * 60);
  const s = Math.round((((normalize24(value) - h) * 60) - m) * 60);
  return `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}`;
}

export function dateFromDecimalHours(baseDate, decimalHours) {
