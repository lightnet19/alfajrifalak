export function applyParallax({ RA, Dec, lat, LST, distanceKm = 384400 }) {
  ...
  const deltaRA = (-sinPi * Math.cos(latRad) * Math.sin(HA)) / Math.cos(decRad);
  const newRA = RA + deltaRA / rad;

  const newDec = Dec + (-sinPi * (Math.sin(latRad) * Math.cos(decRad) - Math.cos(latRad) * Math.sin(decRad) * Math.cos(HA))) / rad;
