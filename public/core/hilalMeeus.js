...
export function getHilalMeeus({ date, lat, lon }) {
  ...
  const moonTopo = applyParallax({ RA: moon.RA, Dec: moon.Dec, lat, LST, distanceKm: moon.distanceKm });

  const sunHor = equatorialToHorizontal(sun.RA, sun.Dec, lat, LST);
  const moonHor = equatorialToHorizontal(moonTopo.RA, moonTopo.Dec, lat, LST);
