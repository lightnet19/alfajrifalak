/**
 * copypdf.js — Salin ke clipboard & Export PDF
 * Al-Fajri v2.2 | Lembaga Falakiyah PCNU Kencong
 * Depends on: math.js, astro.js, prayer.js, ui.js
 */
'use strict';
...
function _cp(text, btn) {
  ...
  }).catch(() => alert('Gagal menyalin. Izinkan akses clipboard di browser.'));
}

function copyHilal(btn) {
  const t = document.getElementById('hilalOut').innerText;
  if (!t || t.startsWith('Klik')) return alert('Hitung hilal terlebih dahulu.');
  _cp(t, btn);
}
...
function _pdfFtr(doc) {
  ...
  doc.text('Al-Fajri v2.2 — Jean Meeus Astronomical Algorithms — Lembaga Falakiyah PCNU Kencong',W/2,H-6,{align:'center'});
}
...
function exportHilalPDF() {
  ...
  raw.split('\n').forEach(line => {
    ...
    const t = line.trim();
    if (t.startsWith('──')) { doc.setFontSize(7.5);doc.setTextColor(200,164,74);doc.setFont('courier','bold');doc.text(line,13,y);y+=4.5; }
    else if (t.startsWith('═')) { doc.setDrawColor(55,50,40);doc.setLineWidth(0.15);doc.line(13,y-1,W-13,y-1);y+=2; }
    else if (line.includes(':')) {
      const ci=line.indexOf(':'),key=line.substring(0,ci+1),val=line.substring(ci+1).trim();
      doc.setFontSize(7.5);doc.setFont('courier','normal');doc.setTextColor(100,92,78);doc.text(key,13,y);
      const vc=val.includes('VISIBLE')||val.includes('✓')?[82,184,130]:val.includes('Not Visible')||val.includes('✗')?[216,92,92]:[205,195,170];
      doc.setTextColor(...vc);doc.setFont('courier','bold');doc.text(val,13+doc.getTextWidth(key)+0.5,y);
...
function exportSholatPDF() {
  ...
  const hij=jdToHijri(jd(now.getFullYear(),now.getMonth()+1,now.getDate()));
  ...
  const bW=(W-28)/4, bH=18, sX=14;
  prs.forEach((pr,i)=>{
    const bx=sX+(i%4)*bW,by=y+Math.floor(i/4)*(bH+3);
  ...
  y+=Math.ceil(prs.length/4)*(bH+3)+8;
...
function exportImsakPDF() {
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF({orientation:'l',unit:'mm',format:'a4'}), now=new Date();
...
function exportEphPDF() {
  const {jsPDF}=window.jspdf, doc=new jsPDF({orientation:'p',unit:'mm',format:'a4'}), now=new Date();
