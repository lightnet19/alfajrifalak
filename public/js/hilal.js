...
function calcHilal(hYear, hMonth, lat, lng, elev, tz) {
  ...
  // FIX v3.1.0: Cari new moon terdekat dengan JD awal bulan target.
...
function renderHilalReport(r) {
  ...
  h+=`<span style="display:block;text-align:center;color:var(--text2);font-size:.72rem">Al-Fajri v3.1.0 — Lembaga Falakiyah PCNU Kencong | Jean Meeus (${MLR.length}+${MB.length} Suku)</span>\n`;
  h+=sep();
  ...
  h+=sep()+`<span style="display:block;text-align:center;color:var(--text3);font-size:.68rem">Al-Fajri v3.1.0 — Jean Meeus — Lembaga Falakiyah PCNU Kencong</span>`;
