...
function renderMap() {
  ...
  drawMap(mapCanvas, points, {
    highlight: { lat: baseInput.lat, lon: baseInput.lon },
  });
  mapInfo.textContent = `Peta visibilitas Indonesia untuk ${baseInput.date.toISOString().slice(0, 10)} siap.`;
}
...
document.getElementById("stopMapBtn").addEventListener("click", () => {
  stopMapAnimation();
  renderMap();
});
